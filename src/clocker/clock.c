...
static _pre_t deviders[3] = {
        {CLOCK_AHB_PRE_1, CLOCK_APB_PRE_1}, // HSI
        {CLOCK_AHB_PRE_1, CLOCK_APB_PRE_1}, // HSE
        {CLOCK_AHB_PRE_1, CLOCK_APB_PRE_2}, // PLL
};
...
clock_status_t clock_start(clock_source_t source, uint32_t hse_clock, clock_pll_mul_t pllmul) {
    ...
    if (target_pllmul != pllmul) {
        ...
        RCC->CFGR = (RCC->CFGR & ~RCC_CFGR_PLLMUL_Msk) | (((uint8_t) target_pllmul - 2) << RCC_CFGR_PLLMUL_Pos);
...
void _clock_set_pre(clock_ahb_pre_t ahb_pre, clock_apb_pre_t apb_pre, uint8_t update) {
    ...
    if (_clock_get_current_source() == CLOCK_SOURCE_PLL) {
        ...
        if (fq > 36000000 && ahb_pre == CLOCK_AHB_PRE_1) {
            ...
            if (apb_pre == CLOCK_APB_PRE_1)
                apb_pre = CLOCK_APB_PRE_2;
        }
    ...
    if (((RCC->CFGR & RCC_CFGR_HPRE_Msk) >> RCC_CFGR_HPRE_Pos != ahb_pre)
        ||
        ((RCC->CFGR & RCC_CFGR_PPRE_Msk) >> RCC_CFGR_PPRE_Pos != apb_pre)
            ) {

        RCC->CFGR = (RCC->CFGR & (~(RCC_CFGR_HPRE_Msk | RCC_CFGR_PPRE_Msk)))
                    | (ahb_pre << RCC_CFGR_HPRE_Pos) | (apb_pre << RCC_CFGR_PPRE_Pos);
        if (update)
...
clock_status_t clock_select_source(clock_source_t src) {
        ...
        case CLOCK_SOURCE_HSE:
            ...
            if (clock_values.HSEClock < 24000000) {
                ...
                FLASH->ACR = (FLASH->ACR & ~FLASH_ACR_LATENCY_Msk) |
                             FLASH_ACR_LATENCY;// больше 48кварцы все равно не ставят
            }
    ...
    _clock_set_pre(deviders[current_source].ahb, deviders[current_source].apb, 0);
    _clock_update();
...
clock_status_t
clock_set_pre(clock_source_t source, clock_ahb_pre_t ahb_pre, clock_apb_pre_t apb_pre) {
    deviders[source].ahb = ahb_pre;
    deviders[source].apb = apb_pre;
    if (source == clock_values.Source)
        _clock_set_pre(deviders[clock_values.Source].ahb, deviders[clock_values.Source].apb, 1);
    return CLOCK_OK;
}
...
uint32_t _pll_calc() {
    ...
    // считаем множитель
    uint8_t pllmul = (uint8_t) ((RCC->CFGR & RCC_CFGR_PLLMUL) >> RCC_CFGR_PLLMUL_Pos);
...
void _clock_update() {
    ...
    if (!(RCC_CFGR_PPRE_2 & RCC->CFGR))
        clock_values.SystemAPBClock = clock_values.SystemAHBClock;
    else {
        uint8_t x = (uint8_t) ((RCC->CFGR & (RCC_CFGR_PPRE_0 | RCC_CFGR_PPRE_1)) >> RCC_CFGR_PPRE_Pos);
        clock_values.SystemAPBClock = clock_values.SystemAHBClock >> (x + 1);
    }
...
void clock_security_isr() {
    if (RCC->CIR & RCC_CIR_CSSF) {
        ...
        clock_flags |= HSE_FAIL;
        clock_source_t current_source = _clock_get_current_source();
        _clock_set_pre(deviders[current_source].ahb, deviders[current_source].apb, 1);
    }
}
